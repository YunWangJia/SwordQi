...
namespace SwordQi
{
    public class SwordQi : MonoBehaviour
    {
        ...
        void Update()
        {
            try
            {
                ...
                if (!GetOriginalDataBool)//第一次生效
                {
                    ...
                    StartCoroutine(modTheWeapon.GetOriginalData());
        ...
        public void DelayJudgment(float time)
        {
            ...
            if(time <= 0.6f)
            {
                ...
                try
                {
                    ...
                    if (!GameSetup.IsSinglePlayer)
                    {
                        SendSwordQi(1, LocalPlayer.Transform.position + new Vector3(0f, 1f, 0f), Camera.main.transform.rotation, qics);
                    }
                    //SendSwordQi(1, LocalPlayer.Transform.position, Camera.main.transform.rotation, qics);
        ...
        public void AddItemsBackpack(Transform GameTran)
        {
            ...
            GameObject x_kat = Instantiate(Beibao, GameTran);//整个背包物品已被做成整体
